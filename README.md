...
- The settings page also provides a macOS login item toggle so the menu bar monitor can launch automatically after login. Signed apps use `SMAppService`; local ad-hoc builds fall back to a user LaunchAgent.
...
The script writes `dist/CodexClaudeLimitBar.app`, generates `Contents/Resources/AppIcon.icns` from the root `icon.png`, copies app resources into `Contents/Resources`, signs the staged bundle ad-hoc, and supports `package`, `run`, `--debug`, `--logs`, `--telemetry`, and `--verify` modes.

For a signed package, pass an installed signing identity:

```bash
CODE_SIGN_IDENTITY="Developer ID Application: Your Name (TEAMID)" ./script/build_and_run.sh package
```
